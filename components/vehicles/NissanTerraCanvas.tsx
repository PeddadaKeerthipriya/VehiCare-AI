...
interface NissanTerraCanvasProps {
...
useGLTF.preload("/models/nissan-terra-vehiCare.glb");
...
function TerraModel({
  ...
  const { scene } = useGLTF("/models/nissan-terra-vehiCare.glb");
  const modelRef = useRef<THREE.Group>(null);

  // Clone scene on load to avoid mutating cached GLTF across component mounts
  const clonedScene = useMemo(() => scene.clone(true), [scene]);

  // Track cloned carpaint_gold body materials specifically
  ...
  useMemo(() => {
    ...
    // 3. Traversal to locate carpaint_gold material for color switching
    ...
    clonedScene.traverse((child) => {
      if ((child as THREE.Mesh).isMesh) {
        ...
        } else if (mesh.material && mesh.material.name === "carpaint_gold") {
    ...
    carpaintMaterialsRef.current = found;

    // 4. Dynamic camera framing calculation based on model bounding box
    const maxDim = Math.max(size.x, size.y, size.z);
    const targetY = size.y * 0.48; // Centered at visual mass height
    const fov = 34;
    const fovRad = (fov * Math.PI) / 180;
    
    // Multiplier 1.02 positions camera so Nissan Terra prominently fills the viewer area safely
    const distance = (maxDim / (2 * Math.tan(fovRad / 2))) * 1.02;
    ...
    if (controlsRef && controlsRef.current) {
      ...
      if (camera) {
        camera.fov = fov;
        // Set near clipping plane extremely small relative to maxDim (0.2mm) to prevent zoom clipping
      ...
      // Allow detailed close-up inspection of headlights, grille, wheels, and door handles
  ...
  // Update color in-memory when selectedColor changes without reloading GLB
  useEffect(() => {
    if (selectedColor && carpaintMaterialsRef.current.length > 0) {
...
export default function NissanTerraCanvas({
  isAutoRotate,
  onLoaded,
  controlsRef,
  selectedColor,
}: NissanTerraCanvasProps) {
  return (
